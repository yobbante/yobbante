...
// ─────────────────────────── Static config ───────────────────────────
// Extracted to keep this file focused on the flow's state machine.
import {
  TIME_SLOTS, GOODS_TYPES, TRANSPORT_MODES, PRIORITIES, PAYMENT_METHODS,
  OPTION_ICONS, fmtEur, corridorRisk,
  type SenderKind, type GoodsId,
} from './send/constants';
...
export function SendFlow({ compactHeader }: { compactHeader?: React.ReactNode } = {}) {
  ...
  const [autoDevis, setAutoDevis]         = useState<import('@/lib/devis').DevisRow | null>(null);
  ...
  async function submit() {
    ...
    try {
      ...
      // ── Devis auto-généré rattaché au dossier (statut "en attente d'envoi").
      // Aucun message WhatsApp de devis n'est envoyé ici : un agent doit
  ...
  if (confirmed) {
    return (
      <SendConfirmation
        ...
        originProfile={originProfile} destProfile={destProfile}
        pickupDate={pickupDate} pickupSlot={pickupSlot} pickupAddress={pickupAddress}
        senderName={senderName} senderPhone={senderPhone}
        recipientName={recipientName} recipientPhone={recipientPhone}
        transportMode={transportMode} priority={priority} insurance={insurance}
        goodsType={goodsType} description={description}
        weight={weight} parcelCount={parcelCount}
      />
    );
  }
