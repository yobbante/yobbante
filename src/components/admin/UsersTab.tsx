            ...
            return (
              <div key={u.id} className="bg-card border border-border rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                ...
                <div className="flex flex-wrap gap-2 sm:justify-end">
                  ...
                  />
                  <RoleToggle label="Agent support" active={hasAgentSupport} disabled={mutateRole.isPending}
                    onAdd={() => mutateRole.mutate({ user_id: u.id, role: 'agent_support', op: 'add' })}
                    onRemove={() => mutateRole.mutate({ user_id: u.id, role: 'agent_support', op: 'remove' })}
                  />
