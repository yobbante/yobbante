...
BEGIN
  ...
  -- Destinataire (notifié en parallèle du client quand renseigné et différent)
  v_rec_phone := NULLIF(btrim(COALESCE(NEW.recipient_phone, '')), '');
  IF v_rec_phone IS NOT NULL
     AND right(regexp_replace(v_rec_phone, '\D', '', 'g'), 9)
         = right(regexp_replace(COALESCE(v_phone, ''), '\D', '', 'g'), 9) THEN
    v_rec_phone := NULL;
  END IF;
  v_rec_prenom := COALESCE(NULLIF(split_part(btrim(COALESCE(NEW.recipient_name, '')), ' ', 1), ''), 'Bonjour');
  ...
  v_eta := CASE WHEN NEW.estimated_delivery_date IS NOT NULL
                ...
                ELSE 'à venir' END;
  v_service := COALESCE(NEW.transport_mode, NEW.app_source, 'Yobbanté');
  ...
  IF TG_OP = 'INSERT' THEN
    IF v_phone IS NOT NULL AND length(btrim(v_phone)) >= 6 THEN
      PERFORM public._wa_send_via_function(
        ...
        jsonb_build_array(v_prenom, COALESCE(NEW.tracking_id, NEW.reference, ''), v_service, v_route, v_weight),
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    IF NEW.status = 'ASSIGNED' AND NEW.assigned_transporteur_ref IS NOT NULL THEN
      ...
      IF v_transporteur.telephone_1 IS NOT NULL THEN
        PERFORM public._wa_send_via_function(
          ...
          jsonb_build_array(
            v_transporteur.prenom_gp, COALESCE(NEW.reference, ''), v_dep_date,
            v_route, v_full_name, v_weight, COALESCE(NEW.contact_phone, '—')
          ),
      ...
      IF v_phone IS NOT NULL AND length(btrim(v_phone)) >= 6 THEN
        PERFORM public._wa_send_via_function(
          ...
          jsonb_build_array(v_prenom, COALESCE(NEW.tracking_id, NEW.reference, ''), v_route, v_dep_date),
          NEW.id, NULL, 'status_assigned_client'
        );
      END IF;
    END IF;

    -- Départ proposé, en attente de la décision du client
    IF NEW.status = 'AWAITING_CLIENT' AND v_phone IS NOT NULL THEN
      PERFORM public._wa_send_via_function(
        'client', v_phone, 'departure_assigned',
        jsonb_build_array(v_prenom, COALESCE(NEW.tracking_id, NEW.reference, ''), v_route, v_dep_date),
        NEW.id, NULL, 'status_awaiting_client'
      );
    END IF;
    ...
    IF NEW.status = 'IN_TRANSIT'
      ...
      PERFORM public._wa_send_via_function(
        ...
        jsonb_build_array(v_prenom, COALESCE(NEW.tracking_id, NEW.reference, ''), COALESCE(NEW.destination_country, ''), v_dep_date, v_eta),
        NEW.id, NULL, 'status_in_transit'
      );
      IF v_rec_phone IS NOT NULL THEN
        PERFORM public._wa_send_via_function(
          'client', v_rec_phone, 'package_in_transit',
          jsonb_build_array(v_rec_prenom, COALESCE(NEW.tracking_id, NEW.reference, ''), COALESCE(NEW.destination_country, ''), v_dep_date, v_eta),
          NEW.id, NULL, 'status_in_transit_recipient'
        );
      END IF;
    END IF;
    ...
    IF NEW.status = 'ARRIVED_HUB' AND v_phone IS NOT NULL THEN
      ...
      IF v_rec_phone IS NOT NULL THEN
        PERFORM public._wa_send_via_function(
          'client', v_rec_phone, 'package_arrived',
          jsonb_build_array(v_rec_prenom, COALESCE(NEW.tracking_id, NEW.reference, ''), COALESCE(NEW.destination_country, '')),
          NEW.id, NULL, 'status_arrived_hub_recipient'
        );
      END IF;
    END IF;
    ...
    IF NEW.status = 'DELIVERED' AND v_phone IS NOT NULL THEN
      ...
      IF v_rec_phone IS NOT NULL THEN
        PERFORM public._wa_send_via_function(
          'client', v_rec_phone, 'package_delivered',
          jsonb_build_array(v_rec_prenom, COALESCE(NEW.tracking_id, NEW.reference, '')),
          NEW.id, NULL, 'status_delivered_recipient'
        );
      END IF;
    END IF;

    -- Destinataire informé dès qu'un départ est confirmé/assigné
    IF NEW.status IN ('ASSIGNED', 'DEPARTURE_CONFIRMED') AND v_rec_phone IS NOT NULL THEN
      PERFORM public._wa_send_via_function(
        'client', v_rec_phone, 'departure_assigned',
        jsonb_build_array(v_rec_prenom, COALESCE(NEW.tracking_id, NEW.reference, ''), v_route, v_dep_date),
        NEW.id, NULL, 'status_departure_recipient'
      );
    END IF;
  END IF;
