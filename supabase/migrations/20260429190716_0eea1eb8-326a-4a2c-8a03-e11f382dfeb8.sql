-- Réduire le pricing global de 25% (1/4) en abaissant la marge plateforme.
-- Avant : v_margin = 1.22 (marge ~22%)
-- Après : v_margin = 0.915 (= 1.22 * 0.75)
-- Toutes les fonctions de quote (calculate_quote, quote-shipment, pricing-calculate,
-- auto_match_shipment) consomment calculate_quote_v2, donc la baisse est uniforme.
...
CREATE OR REPLACE FUNCTION public.calculate_quote_v2(
  ...
  p_transport_mode text DEFAULT 'air'::text,
  p_priority text DEFAULT 'standard'::text,
  p_goods_type text DEFAULT 'standard'::text
)
 RETURNS TABLE(zone_id text, zone_name text, transport_mode text, taxable_weight_kg numeric, volumetric_weight_kg numeric, base_price_xof numeric, weight_cost_xof numeric, raw_price_xof numeric, weight_bracket_mult numeric, goods_mult numeric, urgency_mult numeric, supply_mult numeric, margin_mult numeric, price_xof numeric, price_eur numeric, delivery_days_min integer, delivery_days_max integer, fallback_mode boolean, requires_manual_quote boolean, insurance_required boolean, confidence text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_margin numeric := 0.915; -- réduit de 25% (était 1.22)
