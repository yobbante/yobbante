-- Add flag to suppress trigger notifications for bot-created partial dossiers
ALTER TABLE public.dossiers
  ADD COLUMN IF NOT EXISTS skip_whatsapp_trigger boolean NOT NULL DEFAULT false;

-- Update the trigger function: skip when flag set or when essential fields missing/N/A
...
DECLARE
  ...
  v_weight text;
BEGIN
  -- Hard guard: skip flag (bot sessions, partial drafts)
  IF COALESCE(NEW.skip_whatsapp_trigger, false) = true THEN
    RETURN NEW;
  END IF;

  -- Hard guard: must have real client info before any notif (kills "N/A" alerts)
