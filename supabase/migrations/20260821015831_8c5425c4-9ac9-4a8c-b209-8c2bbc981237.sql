...
begin
  ...
  delete from public.refund_requests where dossier_id = _dossier_id;
  delete from public.edit_tokens where dossier_id = _dossier_id;

  delete from public.dossiers where id = _dossier_id;
end;
$$;

revoke all on function public.admin_delete_dossier(uuid) from public, anon;
grant execute on function public.admin_delete_dossier(uuid) to authenticated, service_role;